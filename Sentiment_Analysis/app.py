...
class EnhancedSentimentAnalyzer:
    ...
    def setup_models(self):
        """Load classification models from saved_models directory."""
        try:
            # Chemin absolu vers le dossier saved_models
            models_dir = r"C:\Users\Ourti\OneDrive\Desktop\Projet\CSh_Tasks\saved_models"

            if not os.path.exists(models_dir):
                st.error(f"Model directory not found: {models_dir}")
                return

            # Mise à jour des noms de fichiers des modèles
            model_files = {
                'vectorizer': 'tfidf_vectorizer.pkl',
                'logistic_regression': 'Logistic_Regression_model.pkl',
                'decision_tree': 'Decision_Tree_model.pkl',
                'random_forest': 'Random_Forest_model.pkl',
                'knn': 'KNearest_Neighbors_model.pkl',
                ...
                try:
                    ...
                    if os.path.exists(model_path):
                        if model_name == 'vectorizer':
                            ...
                            st.success(f"Loaded vectorizer from {filename}")
                        else:
    ...
    def analyze_sentiment(self, text, model_name='TextBlob'):
        ...
        try:
            ...
            # Mise à jour du mapping des noms de modèles vers leurs clés
            model_key_mapping = {
            ...
            if model_name in model_key_mapping:
                model_key = model_key_mapping[model_name]
                if not self.vectorizer or not self.models.get(model_key):
...
def single_text_analysis(analyzer):
    ...
    with col2:
        model_choice = st.selectbox(
            "Choose a model",
            ["TextBlob", "Logistic Regression", "Decision Tree", "Random Forest", "K-Nearest Neighbors", "Naive Bayes"]
        )
